...
- [ ] **0.1** Upgrade Flutter to latest stable (`flutter upgrade`)
- [ ] **0.2** Create Flutter project `lumluay_app` with all platforms enabled
- [ ] **0.3** Setup Serverpod project (`lumluay_server` + `lumluay_client` + `lumluay_flutter`)
- [ ] **0.4** Install & configure core dependencies:
  - `shadcn_flutter`, `drift`, `riverpod`, `go_router`, `slang`
  - `connectivity_plus`, `uuid`, `json_annotation`, `freezed`
- [ ] **0.5** Design system setup:
...
- [ ] **0.6** i18n setup — Lao (ລາວ), Thai (ไทย), English with `slang`
- [ ] **0.7** Drift local database schema v1 (all core tables with migrations)
- [ ] **0.8** Core architecture scaffolding:
...
- [ ] **0.9** Auth module — PIN login per device (offline), account login (online)
- [ ] **0.10** App shell — responsive layout with navigation (sidebar/bottom tabs)
...
- [ ] **1.1** DB tables: `items`, `categories`, `variants`, `variant_groups`, `modifiers`, `modifier_groups`, `item_images`
- [ ] **1.2** Categories screen — CRUD, drag-to-reorder, color/icon picker
- [ ] **1.3** Items list screen — grid/list view toggle, search, filter by category, sort
- [ ] **1.4** Item create/edit screen:
  - Name, SKU, barcode, price, cost, tax group
  - Category assignment
  - Image upload (camera + gallery)
  - Track stock toggle
  - Sold by weight toggle
- [ ] **1.5** Variants system — variant groups (Size, Color) + variant options (S/M/L)
  - Auto-generate variant combinations
  - Per-variant price, SKU, barcode, stock
- [ ] **1.6** Modifiers system — modifier groups (Toppings, Extras)
  - Per-modifier price adjustment
  - Min/max selection rules
- [ ] **1.7** Barcode generation + label printing (item labels with price/barcode)
- [ ] **1.8** CSV import/export for bulk item management
- [ ] **1.9** Item search — full-text search + barcode scan lookup
...
- [ ] **2.7** Customer assignment to ticket
- [ ] **2.8** Hold ticket (save as open ticket, continue later)
- [ ] **2.9** Dining options selector: Dine-in / Take-out / Delivery
...
- [ ] **3.4** PDF receipt generation — for email/share
- [ ] **3.5** Receipt history screen:
...
- [ ] **4.1** DB tables: `inventory_levels`, `stock_adjustments`, `inventory_counts`, `inventory_count_items`, `purchase_orders`, `purchase_order_items`, `transfer_orders`, `transfer_order_items`, `suppliers`
- [ ] **4.2** Inventory dashboard — stock levels overview, low stock alerts
- [ ] **4.3** Stock adjustment screen — reason codes (damaged, lost, correction), quantity +/-
- [ ] **4.4** Inventory count (stock take):
  - Create count session
  - Scan or manually enter counted quantities
  - Compare expected vs counted
  - Apply adjustments
- [ ] **4.5** Low stock notifications — configurable threshold per item, in-app alert
- [ ] **4.6** Purchase orders:
...
- [ ] **4.7** Transfer orders (multi-store):
  - Transfer stock between stores
  - Statuses: pending → in-transit → received
- [ ] **4.8** Inventory history — log all stock movements with timestamp + reason
- [ ] **4.9** Inventory valuation report — total stock value (cost-based)
- [ ] **4.10** Production / composite items:
...
- [ ] **5.1** DB tables: `customers`, `loyalty_transactions`, `loyalty_settings`
- [ ] **5.2** Customer list screen — search, filter, sort
- [ ] **5.3** Customer create/edit — name, phone, email, address, notes, birthday
- [ ] **5.4** Customer detail — purchase history, total spend, loyalty balance
- [ ] **5.5** Loyalty program:
  - Points per currency spent (configurable ratio)
  - Redeem points as discount
  - Points history
- [ ] **5.6** Assign customer to sale (quick search popup on sales screen)
...
- [ ] **6.1** DB tables: `employees`, `employee_roles`, `time_entries`, `role_permissions`
- [ ] **6.2** Roles & permissions:
  - Admin: full access
  - Manager: reports, inventory, items, employees (no settings)
  - Cashier: sales only
  - Custom roles with granular permissions
- [ ] **6.3** Employee list screen — CRUD
- [ ] **6.4** PIN-based employee switching (quick switch on POS screen)
- [ ] **6.5** Time clock — clock in/out, view timecard, calculate hours
- [ ] **6.6** Sales by employee report
...
- [ ] **7.1** DB tables: `shifts`, `cash_movements`
- [ ] **7.2** Open shift — set starting cash amount
- [ ] **7.3** Cash in/out — record reason + amount during shift
- [ ] **7.4** Close shift — enter counted cash, system calculates expected vs actual, show discrepancy
- [ ] **7.5** Shift summary report — sales breakdown by payment method, refunds, cash movements
- [ ] **7.6** Shift history — view past shifts with full detail
...
- [ ] **8.1** Reports dashboard — quick stats (today's sales, items sold, avg ticket)
- [ ] **8.2** Sales summary — by day/week/month/custom range, with charts
- [ ] **8.3** Sales by item — top sellers, revenue per item
- [ ] **8.4** Sales by category
- [ ] **8.5** Sales by employee
- [ ] **8.6** Sales by payment method
- [ ] **8.7** Sales by hour (heatmap for peak hours)
- [ ] **8.8** Tax report — tax collected by rate, by period
- [ ] **8.9** Discount report — total discounts given
- [ ] **8.10** Customer report — top customers, visit frequency
- [ ] **8.11** Inventory reports — stock levels, valuation, low stock
- [ ] **8.12** Expense tracking — record business expenses (rent, utilities, supplies, etc.)
- [ ] **8.13** Profit & Loss report — revenue - COGS - expenses = net profit, by period
- [ ] **8.14** Export all reports to CSV / PDF
...
- [ ] **9.1** DB tables: `stores`, `store_settings`
- [ ] **9.2** Store CRUD — name, address, phone, tax settings, receipt header
- [ ] **9.3** Store selector — switch between stores
- [ ] **9.4** Per-store inventory (stock tracked per location)
- [ ] **9.5** Reports — filter by store or view combined
- [ ] **9.6** Per-store receipt templates & printer configs
- [ ] **9.7** Per-store currency settings (primary + accepted currencies)
...
- [ ] **10.1** General settings — store name, address, phone, logo, currency
- [ ] **10.2** Tax settings — CRUD tax rates, assign to items/categories, inclusive/exclusive
- [ ] **10.3** Receipt template settings — header text, footer text, show/hide fields
- [ ] **10.4** Printer management — add/test/remove printers (Bluetooth/WiFi/USB)
- [ ] **10.5** Cash drawer settings — enable/disable auto-open
- [ ] **10.6** Payment method settings — enable/disable cash, QR, custom methods
- [ ] **10.7** Currency settings — primary currency, exchange rates, accepted currencies
- [ ] **10.8** Language selector (Lao/Thai/English)
- [ ] **10.9** Theme selector (Light/Dark)
- [ ] **10.10** Audit log viewer — searchable log of all system actions
- [ ] **10.11** Backup & restore — export/import local database
- [ ] **10.12** About screen — app version, licenses
...
- [ ] **11.1** Serverpod data models — mirror all local tables
- [ ] **11.2** API endpoints:
...
- [ ] **11.5** SaaS subscription management:
...
- [ ] **13.1** Customer Display System (CDS):
...
- [ ] **14.1** Unit tests — all business logic (tax calc, discount, stock, loyalty, currency)
- [ ] **14.2** Widget tests — critical screens (sales, payment, items)
- [ ] **14.3** Integration tests — full sale flow, sync flow
- [ ] **14.4** Security:
...
- [ ] **14.6** Error tracking — Sentry or similar crash reporting
- [ ] **14.7** VPS deployment:
...
- [ ] **14.9** CI/CD pipeline (optional) — GitHub Actions for automated builds
...
|-------|-------|---------------|----------|--------|
| 1 | Phase 0: Foundation | 2 weeks | CRITICAL | 🔄 IN PROGRESS |
| 2 | Phase 1: Items & Categories | 2 weeks | CRITICAL | ⬜ TODO |
| 3 | Phase 2: Sales Screen | 2 weeks | CRITICAL | ⬜ TODO |
| 4 | Phase 3: Payment & Receipts | 2 weeks | CRITICAL | ⬜ TODO |
| 5 | Phase 7: Cash Mgmt & Shifts | 1 week | CRITICAL | ⬜ TODO |
| 6 | Phase 4: Inventory | 2 weeks | HIGH | ⬜ TODO |
| 7 | Phase 5: Customers & Loyalty | 1 week | HIGH | ⬜ TODO |
| 8 | Phase 6: Employees | 1 week | HIGH | ⬜ TODO |
| 9 | Phase 8: Reports & P&L | 2 weeks | HIGH | ⬜ TODO |
| 10 | Phase 9: Multi-Store | 1 week | HIGH | ⬜ TODO |
| 11 | Phase 10: Settings | 1 week | HIGH | ⬜ TODO |
| 12 | Phase 11: Backend & Sync | 3 weeks | HIGH | ⬜ TODO |
| 13 | Phase 12: Restaurant | 2 weeks | MEDIUM | ⬜ TODO |
| 14 | Phase 13: Additional | 2 weeks | MEDIUM | ⬜ TODO |
| 15 | Phase 14: Hardening | 2 weeks | CRITICAL | ⬜ TODO |
